...
void destroyIntTree(IntTree *aTree) //видаляємо дерево
{
  if (NULL != aTree)
  {
    destroyNode(aTree->root); //викликаємо destroyNode, коли всі ноди видаляться
...
{
  IntNode *theCurrentNode = NULL; //Якщо функція пошуку нічого не знайшла то вона повертає нуль, а якщо знайшла, то повертатиме посилання на ноду
...
void deleteNodeWithValue(IntTree *aTree, int aValue) {
  ...
  //визначаємо чи є ця нода кінцевою. листові ноди
...
//робимо нулі для батьківського елемента
  ...
  //цей цикл виконується, якщо нода має два нащадки
