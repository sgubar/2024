...
void deleteElement(Tree *tree, VALTYPE num) {
	...
	if (current->left == NULL && current->right == NULL) {			// node is leaf
		...
		*currentPtr = (current->right == NULL ? current->left : current->right); // save connection for correspondent branch
		...
		TreeNode **currentPtrL = &(current->right);					// pointer to branch connection

		// 1. find minimal (actually, most left) element in in right branch
		while ((*currentPtrL)->left != NULL) {						// is there more left element than current?
			currentPtrL = &((*currentPtrL)->left);					// save connection to left element
		}

		// 2. set current value to successor value
		current->num = (*currentPtrL)->num;

		// 3. successors' parent left element -> right child of successor
		TreeNode *buf = *currentPtrL; 								// save pointer to successor for later deletion
		*currentPtrL = (*currentPtrL)->right;						// replace connection with connection of new element
		free(buf);
	}
}
