...
void display()
{
    ...
    glBegin(GL_QUADS); // Піраміда з основою правильного 4-кутника складається з 5 гранів (чотирикутника і трикутників)

    // Ліва грань (x = -sqrt(2)/2)
    ...
    glBegin(GL_TRIANGLES);// Завершення малювання квадратної грані піраміди
    // Верхня грань (y = 1.0f)
    ...
    glEnd(); // Завершення малювання трикутних гранів піраміди
...
{
    // Якщо натискаємо стрілку вгору або вниз - фігура рухається по осі Ох, якщо право або вліво - рухається по осі Оу
    if (sk == GLUT_KEY_UP) {
        a_x += 0.50f;;
    }
