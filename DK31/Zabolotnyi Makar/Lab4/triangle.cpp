...
void destroyTriangle(Triangle *triangle){
  if (NULL != triangle) {
		for (int i = 0; i < NUM; i++){// створює цикл в якому кожна точка в трикутника видаляється
            destroyPoint(triangle->points[i]);
        }
        deleteColor(triangle->color); // видалення кольору трикутника
	}
...
//створення двозв'язного списка трикутників
