...
int t = 1000 / 4; // Період обертання фігури навколо початку координат (в даному прикладі робимо 1 оберт за 24 кадри/с)
float theta_spd = 360.0f / t; // Кут оберту між кадрами
float theta = 0.0f; // Кут оберту як глобальна змінна
RTrgleList_s* figlist; // Наш список фігур
...
int main(int argc, char* argv[])
{
    ...
    Point* points3[3];
    Point* points4[3];
    RTrgle *figure1, *figure2, *figure3, *figure4;

    // Створимо список фігур
    figlist = createFigList();

    // Створимо фігури і додамо їх до списку
    points1[0] = createPoint(0, 80);
    points1[1] = createPoint(50, 80);
    points1[2] = createPoint(50, 70);
    figure1 = createRTrgle(points1, createColor(1.0, 0.0, 0.0));
    ...
    figure2 = createRTrgle(points2, createColor(1.0, 1.0, 0.0));

    addFigToFigList(figlist, figure2);

    points3[0] = createPoint(2, 74);
    points3[1] = createPoint(45, -89);
    points3[2] = createPoint(-56, -89);

    figure3 = createRTrgle(points3, createColor(0.5, 0.5, 0.5));
    ...
    figure4 = createRTrgle(points4, createColor(0.0, 0.0, 1.0));
