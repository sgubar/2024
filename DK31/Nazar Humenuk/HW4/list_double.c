...
void destroyFirstDoubleInLIst(ListDoubl *list){//функція видалення елементу зі списку за індексом
  if (NULL == list ) {// якщо списка не існує 
    return ;
  }
  NodeDoubl *nodeD = list->head;//створюємо вказівник на перший елемент списку
...
void destroyLastDoubleInLIst(ListDoubl *list){
   if (NULL == list ) {// якщо списка не існує 
    return ;
  }
  NodeDoubl *nodeD = list->head;//створюємо вказівник на перший елемент списку
  unsigned int i = 0;
  while ( i != list->count-2) {// цикл який перебирає елементи до передостанього елементу
    nodeD = nodeD->next;//перехід на наступний елемент
    i ++;
  }
    free(nodeD->next);//звільняємо память для елементу за індексом
    nodeD->next = NULL;//вводимо як наступний елемент NULL
    list->tail = nodeD;
  list->count--;// зменшуємо кількіть елементів у списку
